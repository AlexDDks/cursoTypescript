...
//COMENZAMOS A UTILIZAR LA INTERFACE, REPETIMOS EL CÓDIGO ANTERIOR PARA UTULIZAR ESE EJEMPLO:
...
////Acá he comentado las otras propiedades de Vehicle, para poder utilizar la función printVehicle1 para declarar drink, ya que drink tiene otras propiedades, pero en ella se encuentra summary, lo cual comparte con el objeto oldCivi1
  ...
  //EN LA PARTE 2 DEL CURSO DE INTERFACES hemos agregado otra propiedad la cual es una función que devuelve un string, que obligatoriamente debe de estar presente en el objeto en donde se vaya a requerir la interfaz. La anotación hace alusión a que tendremos un return string
  summary():string;
...
//Cambio el nombre de la variable debido a que estoy reutilizando código y estas son variables en un mismo bloque y en este caso constante
const oldCivic1={
  nombre: 'Civic',
  año: 2000,
  funciona: false,
...
//Hemos agregado un nuevo objeto el cual también tiene un tipo de dato que es una función que devuelve un string, por lo que también es considerado de tipo "Vehicle", que por el momento vamos a saltarnos el que el nombre de la interface no concuerda con la información:
const drink={
  color: "negro",
  carbonatado: true,
  azúcar:40,
...
//Como podemos constatar en esta parte del código. Si lo comparamos con la línea 11, podemos ver que únicamente estamos utilizando la interface Vehicle y no declarando el tipo de cada valor, como comúnmente sucedía
...
// Como vemos, el printVehicle1 espera argumentos de tipo Vehicle y dado que oldCivic1 y drink tienen en sus propiedades "summary", entonces pertenecen a ese tipo, sin importanr si en sus filas cuentan con más propiedades.
 ...
 //EN RESUMEN:
 //Tenemos una función printVehicle1 cuyos argumentos son de tipo Vehicle, por lo que los objetos oldCivic1 y drinks deben satisfacer a la interfaz Vehicle para trabajar con la función printVehicle1. Esot nos sirve como una estrategia para  generalizar nuestro código y ahorrarnos líneas y rehusar más. Por lo que todas las funciones que creemos van a aceptar interfaces tanto como sea posible, SIEMPRE QUE SEA NECESARIO